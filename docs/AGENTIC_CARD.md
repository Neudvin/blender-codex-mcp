...
business-card concept through ten typed MCP tools. Blender constructs the
...
Blender and Codex. The connection should report version `0.2.0`. Tests use
Blender 4.5.3 LTS; a session reporting Blender 3.6.2 is outside that tested version.
...
4. `card_save(expected_filepath=...)` saves all project contents in the **same**
...
zero. Independently scripted recessed models are **not** automatically adopted
as generated recipes: normal `card_save` can preserve such a loaded project, but
typed card geometry edits still require a compatible tracked asset. If different
features exist in different `.blend` files, reconcile those files deliberately
before continuing. Do not regenerate a layered card over a custom recessed model.
